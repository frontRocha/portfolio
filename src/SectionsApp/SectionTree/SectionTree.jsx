        ...
        <section id='section3' className="bg-[#F0F0F0] min-h-screen w-full fontMont">
            ...
            <div className='max-w-[1000px] px-10 mx-auto pb-40'>
                <div className='grid grid-cols-1 md:grid-cols-2 place-items-center gap-6'>
                    {Pages.map((item, key) => (
                        <div key={key} className={`${expandedCardId === item.id ? 'h-auto min-h-auto' : 'h-[450px] min-h-[450px]'} h-auto flex flex-col justify-between bg-white w-[320px] lg:w-[416px] py-4 px-4 rounded-xl shadow-xl duration-300`}>
                            <div>
                            ...
                            <div className={`${expandedCardId === item.id ? 'opacity-100 duration-1000 visible relative' : 'opacity-0 invisible absolute'}`}>
                                <h5>Tecnologias usadas:</h5>
                                <div className='grid grid-cols-2 md:grid-cols-3 place-items-center relative'>
